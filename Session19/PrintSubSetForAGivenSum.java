class Sample
{
	...
	public void printSubSet(int[] arr, int sum , Boolean[][] dp)
	{
		int i = arr.length-1; 
		while( i > 0 &&  sum > 0)
		{
			if(arr[i] <= sum)
			{
				if(dp[i-1][sum] == false && dp[i][sum] == true)
						{
				System.out.print(arr[i]+" ~ ");
				sum = sum-arr[i];
						}else if(dp[i-1][sum] == true && dp[i-1][sum-arr[i]] == true)
					{
				System.out.print(arr[i]+" ~ ");
				sum = sum-arr[i];
					}
...
public class PrintSubSetForAGivenSum
{
